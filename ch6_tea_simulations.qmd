...
out_high_n <- t.test(rating ~ condition, data = tea_data_highn, var.equal = TRUE) 
```
...
tea_data_summary <- samps |> 
  ...
  summarise(mean_rating = mean(rating)) |> # get mean rating
  group_by(sim) |> 
  summarise(delta = mean_rating[condition == "milk first"] -
              mean_rating[condition == "tea first"]) #calculate difference for each sim.
```
...
tea_data_highn_summary <- samps_highn |>
  ...
  group_by(sim) |> 
  summarise(delta = mean_rating[condition == "milk first"] - mean_rating[condition == "tea first"])
```
...
There is a difference in how narrow the distribution is. When we sample more data, the distribution becomes narrower as there is less sample noise
...
simulated data for each session but here we reshuffle data labels so it's going to be a null distribution when we plot a histogram across all sessions
...
```{r}
# look centered around 0. When we shuffle the label, we make an assumption that there is no difference between two conditions so the delta should be around 0. 
```
